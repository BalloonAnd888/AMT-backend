...
from preprocessing.constants import DATA_PATH, N_KEYS, HOP_LENGTH, SAMPLE_RATE, N_MELS, MEL_FMAX, MEL_FMIN, WINDOW_LENGTH, SEQUENCE_LENGTH
...
class TrainConfig:
    ...
    # Data loader
    TRAIN_BS: int = 16          # lower than original (40) since mels computed on-the-fly
...
# ##############################################################################
def _collate_fn_impl(batch, mel_extractor):
    """
    Helper function for collating a batch.
    1. Stacks raw audio samples into a batch
    2. Runs the mel extractor on the batch
    3. Returns (logmels, onset_rolls, velocity_rolls)
    """
    ...
    # Compute log-mel on CPU (mel_extractor stays on CPU for DataLoader workers)
    with torch.no_grad():
        logmels = mel_extractor(audio.cpu())  # (B, n_mels, t)

    return logmels, onsets.cpu(), velocities.cpu()

def make_collate_fn(mel_extractor):
    """
    ...
    3. Returns (logmels, onset_rolls, velocity_rolls)
    """
    return functools.partial(_collate_fn_impl, mel_extractor=mel_extractor)
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    # Mel extractor (CPU — used inside DataLoader workers)
    # ------------------------------------------------------------------
    # CPU (for DataLoader collate)
    mel_extractor_cpu = MelSpectrogram(
    ...
    # GPU (for XV inference)
    mel_extractor_gpu = MelSpectrogram(
        sample_rate=CONF.SAMPLE_RATE,
        window_length=CONF.WIN_SIZE,
        hop_length=CONF.HOP_SIZE,
        n_mels=CONF.N_MELS,
    ...
    train_dataset = MAESTRO(
        CONF.DATA_PATH, groups=['train'],
        sequence_length=SEQUENCE_LENGTH)

    print("Loading validation dataset...")
    xv_dataset = MAESTRO(
        CONF.DATA_PATH, groups=['validation'],
        sequence_length=None)   # full files for XV

    collate_fn = make_collate_fn(mel_extractor_cpu)
    ...
    def xv_single_file(sample, thresholds):
        ...
        with torch.no_grad():
            mel = mel_extractor_gpu(audio)   # (n_mels, t)
    ...
    for epoch in range(1, CONF.NUM_EPOCHS + 1):
        ...
        for batch_idx, (logmels, onsets_raw, velocities_raw) in enumerate(train_dl):
            ...
            # Forward + loss
            # --------------------------------------------------------------
            logmels = logmels.to(DEVICE)               # (B, n_mels, t)
        ...
        epoch_path = os.path.join(model_snapshot_dir,
                                  f"OnsetsAndVelocities_epoch={epoch}.pt")
